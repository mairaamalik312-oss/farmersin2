...
public class supplier_products {
    ...
    public boolean addSupplierProduct(SupplierProduct sp) throws SQLException {
        ...
        String query =
                "INSERT INTO supplier_products " +
                        "(supplier_id, product_id, price_per_unit, " +
                        "available_quantity, minimum_order_quantity, " +
                        "unit_type, quality_grade, " +
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (sp.getQualityGrade() != null
                    && !sp.getQualityGrade().trim().isEmpty()) {
    ...
    // 3. Get Approved Listings by Product ID
    public List<SupplierProduct> getApprovedByProductId(
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    list.add(
                            mapResultSetToSupplierProduct(rs)
                    );
                }
            }
        }
    ...
    // 4. Get All Listings by Supplier ID
    ...
    // 8. Delete Supplier Product
    public boolean deleteSupplierProduct(
            int supplierProductId
    ) throws SQLException {
    ...
    // Helper Method
    private SupplierProduct mapResultSetToSupplierProduct(
            ResultSet rs
    ) throws SQLException {
        ...
        sp.setAvailableQuantity(
                rs.getBigDecimal("available_quantity")
        );

        sp.setMinimumOrderQuantity(
        ...
        sp.setProductionOrHarvestDate(
                rs.getDate("production_or_harvest_date")
        );
