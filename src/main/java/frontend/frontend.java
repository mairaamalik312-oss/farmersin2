...
public class frontend {
    ...
    private void showSignup() {
        ...
        Runnable submit = () -> {
            ...
            if (name.isBlank()) {
                alert("Name required", "Please enter your full name.");
            ...
            if (password.length() < 6) {
                alert("Password too short", "Please choose a password with at least 6 characters.");
                passwordField.requestFocus();
                return;
            }
            if (!password.equals(confirm)) {
                alert("Passwords don't match", "Please make sure both password fields match.");
    ...
    private void showDashboard() {
        ...
        if (role.equals("ADMIN")) {
            HBox quick = new HBox(14,
                    ...
                    actionCard("Marketplace", "View supplier product updates", this::showProducts));
    ...
    private String roleMessage() {
        if (role.equals("ADMIN"))
            return "Approve buyers and suppliers, view product and listing updates, and supervise payments, complaints, refunds and deliveries.";
        if (role.equals("BUYER"))
            return "Browse approved produce, review your orders and payments, and keep track of notifications.";
    ...
    private Node approvalPane(boolean buyer) {
        ...
        if (!buyer) {
            ...
            editRow.setOnAction(e -> {
                ...
                if (row == null) {
                    alert("Select a request", "Choose a supplier request first.");
                    return;
                }
    ...
    private void updateApproval(TableView<Object> table, boolean buyer, String status, Runnable reload) {
        Object row = table.getSelectionModel().getSelectedItem();
        if (row == null) {
            alert("Select a request", "Choose a buyer or supplier request first.");
            return;
        }
        String getter = buyer ? "getBuyerId" : "getSupplierId";
        Integer id = extractInt(row, getter);
        if (id == null) {
            alert("Cannot identify request", "The selected record does not expose " + getter + "().");
            return;
        }
    ...
    private void showCategories() {
        ...
        active.setOnAction(e -> {
            ...
            if (row == null) {
                alert("Select a category", "Choose a category first.");
                return;
            }
    ...
    private void showProducts() {
        boolean adminView = role.equals("ADMIN");

        setHeader(
                "Products",
                adminView
                        ? "View the latest product catalogue and supplier updates"
        ...
        // ADMIN IS VIEW-ONLY FOR PRODUCTS.
        // Suppliers are the users who create and update product records.
        if (role.equals("SUPPLIER")) {
            Button add = primaryButton("Add Product");
            add.setOnAction(e -> modelEditor(
                    "Product",
                    null,
                    obj -> invokeAndReload("ProductService", "addProduct", obj, reload)
            ));

            Button edit = secondaryButton("Update Selected Product");
            edit.setOnAction(e -> {
                Object row = table.getSelectionModel().getSelectedItem();
                if (row == null) {
                    alert("Select a product", "Choose a product to update.");
                    return;
                }
                modelEditor(
                        "Product",
                        row,
                        obj -> invokeAndReload("ProductService", "updateProduct", obj, reload)
                );
            });

            actions.getChildren().addAll(add, edit);
        }

        Label permissionNote = new Label(
                adminView
                        ? "View only: product creation and updates are handled by suppliers. Use Refresh to see the latest changes."
                        : "Create the product here, then use My Listings to add or update its supplier listing details."
        );
        permissionNote.setTextFill(Color.web(MUTED));
        permissionNote.setWrapText(true);

        root.getChildren().addAll(actions, permissionNote);

        if (adminView) {
            // Admin can inspect the current products and the supplier listing
            // information linked to a selected product, but cannot modify either.
            TableView<Object> listings = dataTable();
            Label listingState = new Label("Select a product to view its supplier listing updates.");

            table.getSelectionModel().selectedItemProperty().addListener((obs, oldRow, row) -> {
                if (row == null) return;
                Integer productId = firstInt(row, "getProductId", "getId");
                if (productId != null) {
                    loadInto(
                            listings,
                            listingState,
                            "supplier_products",
                            "getApprovedByProductId",
                            productId
                    );
                }
            });
            ...
            SplitPane split = new SplitPane(
                    wrappedTable("Product Catalogue", table, state),
                    wrappedTable("Supplier Listing Updates", listings, listingState)
            );
            split.setDividerPositions(.52);
            VBox.setVgrow(split, Priority.ALWAYS);
            root.getChildren().add(split);
        } else {
            root.getChildren().addAll(state, table);
            VBox.setVgrow(table, Priority.ALWAYS);
        }
    ...
    private void showSupplierListings() {
        ...
        Button refresh = secondaryButton("Refresh");
        refresh.setOnAction(e -> reload.run());
        Button add = primaryButton("Add Product Listing");
        add.setOnAction(e -> modelEditor("SupplierProduct", null, obj -> {
            setIfPossible(obj, "setSupplierId", currentProfileId);
            invokeAndReload("supplier_products", "addSupplierProduct", obj, reload);
        }));
        Button edit = secondaryButton("Update Selected Listing");
        edit.setOnAction(e -> {
            Object row = table.getSelectionModel().getSelectedItem();
            if (row == null) {
                alert("Select a listing", "Choose a supplier listing first.");
                return;
            }
    ...
    private void showNotifications() {
        ...
        Button refresh = secondaryButton("Refresh");
        refresh.setOnAction(e -> reload.run());
        Button allRead = primaryButton("Mark All Read");
        allRead.setOnAction(e -> runAsync(() -> invoke("NotificationService", "markAllAsReadForUser", currentUserId), x -> reload.run(), this::showError));
        root.getChildren().addAll(new HBox(10, refresh, allRead), state, table);
        VBox.setVgrow(table, Priority.ALWAYS);
        show(root);
        reload.run();
    }
    ...
    private void showPayments() {
        ...
        searchByIdPanel("PaymentService", "getPaymentById", "Payment ID", (table, row) -> {
            ...
            verify.setOnAction(e -> {
                ...
                if (id != null)
                    runAsync(() -> invoke("PaymentService", "verifyPayment", id, adminId, "VERIFIED"), x -> toast("Payment verified", false), this::showError);
            });
            return verify;
        });
    }
    ...
    private void showComplaints() {
        ...
        resolve.setOnAction(e -> {
            ...
            d.setHeaderText("Admin response");
            d.setContentText("Resolution note:");
        ...
        root.getChildren().addAll(new HBox(10, new Label("Status:"), status, resolve), state, table);
        VBox.setVgrow(table, Priority.ALWAYS);
        show(root);
        reload.run();
    }
    ...
    private void showRefunds() {
        ...
        root.getChildren().addAll(new HBox(10, approve, reject), state, table);
        VBox.setVgrow(table, Priority.ALWAYS);
        show(root);
        reload.run();
    }
    ...
    private void showDeliveries() {
        ...
        root.getChildren().addAll(new HBox(10, new Label("Status:"), status, dispatch), state, table);
        VBox.setVgrow(table, Priority.ALWAYS);
        show(root);
        reload.run();
    }
    ...
    private void simpleServiceTable(String service, String method, Object... args) {
        ...
        refresh.setOnAction(e -> reload.run());
        root.getChildren().addAll(refresh, state, table);
        VBox.setVgrow(table, Priority.ALWAYS);
        show(root);
        reload.run();
    }
    ...
    private void searchByIdPanel(String service, String method, String prompt, RowActionFactory factory) {
        ...
        id.setPromptText(prompt);
        id.setPrefWidth(220);
        styleInput(id);
        Button search = primaryButton("Search");
        TableView<Object> table = dataTable();
        Label state = new Label("Enter an ID to search.");
        search.setOnAction(e -> {
            Integer value = safeInt(id.getText());
            if (value == null) {
                alert("Invalid ID", "Enter a valid numeric ID.");
                return;
            }
        ...
        root.getChildren().addAll(bar, state, table);
        VBox.setVgrow(table, Priority.ALWAYS);
        show(root);
    }
    ...
    private CRUDView genericCrudView(String service, String listMethod, String model, boolean includeRefresh, ActionDef... defs) {
        ...
        for (ActionDef d : defs) {
            ...
            b.setOnAction(e -> {
                ...
                if (d.method.startsWith("update") && selected == null) {
                    alert("Select a record", "Choose a row first.");
                    return;
                }
    ...
    private int getterPriority(String n) {
        ...
        if (s.endsWith("id")) return 0;
        if (s.contains("name")) return 1;
        if (s.contains("status")) return 2;
        if (s.contains("email")) return 3;
        return 10;
    }
    ...
    private String propertyName(Method g) {
        ...
        else if (n.startsWith("is")) n = n.substring(2);
        return n;
    }
    ...
    private GridPane buildDetailGrid(Object obj) {
        ...
        for (Method m : readableGettersAll(obj.getClass())) {
            Label k = new Label(pretty(propertyName(m)));
            k.setStyle("-fx-font-weight:bold; -fx-text-fill:" + INK + ";");
            Label v = new Label(String.valueOf(safeGetter(obj, m)));
            v.setWrapText(true);
            v.setMaxWidth(450);
            v.setTextFill(Color.web(INK));
            grid.add(k, 0, r);
            grid.add(v, 1, r++);
        }
        return grid;
    ...
    private List<Method> readableGettersAll(Class<?> c) {
        List<Method> out = new ArrayList<>();
        for (Method m : c.getMethods())
            if (m.getParameterCount() == 0 && m.getDeclaringClass() != Object.class && ((m.getName().startsWith("get") && m.getName().length() > 3) || (m.getName().startsWith("is") && m.getName().length() > 2)))
                out.add(m);
    ...
    private void modelEditor(String simpleModelName, Object existing, Consumer<Object> onSave) {
        try {
            ...
            dialog.setTitle((existing == null ? "Add " : "Edit ") + pretty(simpleModelName));
            ...
            Map<Method, Control> fields = new LinkedHashMap<>();
            int row = 0;
            for (Method setter : editableSetters(cls)) {
                ...
                Label lbl = new Label(pretty(prop));
                lbl.setTextFill(Color.web(INK));
                grid.add(lbl, 0, row);
                grid.add(field, 1, row++);
                fields.put(setter, field);
            }
            ScrollPane sp = new ScrollPane(grid);
            sp.setFitToWidth(true);
            sp.setPrefViewportWidth(560);
    ...
    private Method findGetter(Class<?> cls, String prop) {
        for (String n : List.of("get" + prop, "is" + prop))
            try {
    ...
    private Object safeInvokeGetter(Object obj, Method getter) {
        try {
    ...
    private Control createEditor(Class<?> type, Object value) {
        ...
        if (type.isEnum()) {
            ...
            for (Object e : type.getEnumConstants()) cb.getItems().add(String.valueOf(e));
            if (value != null) cb.setValue(String.valueOf(value));
            return cb;
        }
        TextField f = new TextField(value == null ? "" : String.valueOf(value));
        styleInput(f);
        return f;
    }
    ...
    private java.sql.Date parseSqlDate(String s) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Invalid date '" + s + "'. Use format: yyyy-MM-dd (e.g. 2026-08-22)"
            );
        }
    }
    ...
    private Object readEditor(Control c, Class<?> type) {
        ...
        if (type == Date.class) return parseSqlDate(s);
    ...
    private void invokeAndReload(String service, String method, Object obj, Runnable reload) {
        runAsync(() -> invoke(service, method, obj), x -> {
            toast("Saved successfully", false);
            reload.run();
        }, this::showError);
    }
    ...
    private Object invoke(String serviceSimpleName, String methodName, Object... args) {
        try {
            ...
            if (method == null)
                throw new NoSuchMethodException(serviceClass.getName() + "." + methodName + " with " + args.length + " argument(s)");
            return method.invoke(service, args);
        } catch (InvocationTargetException e) {
            Throwable cause = e.getCause() == null ? e : e.getCause();
            throw new RuntimeException(cause.getMessage() == null ? cause.toString() : cause.getMessage(), cause);
    ...
    private Method findCompatibleMethod(Class<?> cls, String name, Object[] args) {
        ...
        for (Method m : cls.getMethods()) {
            ...
            for (int i = 0; i < pts.length; i++)
                if (args[i] != null && !wrap(pts[i]).isAssignableFrom(wrap(args[i].getClass()))) continue outer;
            return m;
        }
        return null;
    }
    ...
    private void runAsync(Supplier<Object> work, Consumer<Object> success, Consumer<Throwable> fail) {
        ...
        Thread th = new Thread(task, "farmersin-ui-db");
        th.setDaemon(true);
        th.start();
    }
    ...
    private void updateSimpleStatus(TableView<Object> table, String service, String method, String idGetter, String status, Runnable reload) {
        ...
        Integer id = firstInt(row, idGetter, "getId");
        if (id == null) return;
        runAsync(() -> invoke(service, method, id, status), x -> {
    ...
    private void updateSingleId(TableView<Object> table, String service, String method, String idGetter, Runnable reload) {
        ...
        Integer id = firstInt(row, idGetter, "getId");
        if (id == null) return;
        runAsync(() -> invoke(service, method, id), x -> {
            toast("Updated successfully", false);
            reload.run();
        }, this::showError);
    }
    ...
    private Integer extractInt(Object obj, String getter) {
        ...
        try {
            ...
            return v instanceof Number ? ((Number) v).intValue() : safeInt(String.valueOf(v));
        } catch (Exception e) {
            return null;
        }
    }

    private Integer firstInt(Object obj, String... getters) {
        for (String g : getters) {
            Integer i = extractInt(obj, g);
            if (i != null) return i;
        }
        return null;
    }

    private Boolean firstBoolean(Object obj, String... getters) {
        for (String g : getters)
            try {
    ...
    private void setIfPossible(Object obj, String setter, Object value) {
        try {
            Method m = findCompatibleMethod(obj.getClass(), setter, new Object[]{value});
    ...
    private Node wrappedTable(String title, TableView<Object> table, Label state) {
        VBox v = new VBox(8);
        Label h = new Label(title);
        h.setStyle("-fx-font-weight:bold;-fx-font-size:15px;-fx-text-fill:" + INK + ";");
        v.getChildren().addAll(h, state, table);
        VBox.setVgrow(table, Priority.ALWAYS);
    ...
    private VBox panel(String title, String text) {
        ...
        h.setTextFill(Color.web(INK));
        Label b = new Label(text);
        b.setWrapText(true);
        b.setTextFill(Color.web(MUTED));
        v.getChildren().addAll(h, b);
        return v;
    }

    private Node actionCard(String title, String text, Runnable action) {
        VBox v = panel(title, text);
        v.setMinWidth(250);
        v.setOnMouseClicked(e -> action.run());
        v.setStyle(cardStyle() + "-fx-cursor:hand;-fx-border-color:" + WHEAT + ";-fx-border-width:1.4;");
        return v;
    }
    ...
    private Button primaryButton(String text) {
        Button b = new Button(text);
        b.setStyle("-fx-background-color:" + SOIL_DARK + ";-fx-text-fill:" + WHEAT_SOFT + ";-fx-font-weight:bold;-fx-padding:10 16;-fx-background-radius:8;-fx-cursor:hand;");
    ...
    private Button secondaryButton(String text) {
        Button b = new Button(text);
        b.setStyle("-fx-background-color:" + CARD + ";-fx-text-fill:" + INK + ";-fx-font-weight:bold;-fx-padding:9 15;-fx-background-radius:8;-fx-border-radius:8;-fx-border-color:" + BORDER + ";-fx-cursor:hand;");
    ...
    private Button dangerButton(String text) {
        Button b = new Button(text);
        b.setStyle("-fx-background-color:" + DANGER_BG + ";-fx-text-fill:" + DANGER_FG + ";-fx-font-weight:bold;-fx-padding:9 15;-fx-background-radius:8;-fx-cursor:hand;");
        return b;
    }

    private void styleInput(TextInputControl f) {
        f.setStyle("-fx-background-color:" + CARD + ";-fx-text-fill:" + INK + ";-fx-border-color:" + BORDER + ";-fx-border-radius:8;-fx-background-radius:8;-fx-padding:9 11;");
    }
    ...
    private String navStyle(boolean active) {
        return active ? "-fx-background-color:" + WHEAT + ";-fx-text-fill:" + INK + ";-fx-font-weight:bold;-fx-background-radius:8;-fx-padding:10 13;-fx-cursor:hand;" : "-fx-background-color:transparent;-fx-text-fill:" + WHEAT_SOFT + ";-fx-background-radius:8;-fx-padding:10 13;-fx-cursor:hand;-fx-font-weight:bold;";
    }

    private String cardStyle() {
        return "-fx-background-color:" + CARD + ";-fx-background-radius:14;-fx-border-radius:14;-fx-border-color:" + BORDER + ";";
    }

    private DropShadow softShadow() {
        DropShadow ds = new DropShadow();
        ds.setColor(Color.web(SOIL_DARK, 0.16));
        ds.setRadius(18);
        ds.setOffsetY(6);
        return ds;
    }
    ...
    private void alert(String title, String message) {
        ...
        a.setTitle("FarmersIn");
        a.setHeaderText(title);
        a.setContentText(message);
        a.getDialogPane().setStyle("-fx-background-color:" + CARD + ";");
        a.showAndWait();
    }
    ...
    private void showError(Throwable ex) {
        ...
        a.setTitle("FarmersIn");
        a.setHeaderText("Unable to complete this action");
    ...
    private void toast(String message, boolean error) {
        alert(error ? "Action failed" : "Success", message);
    }
    ...
    private static class ActionDef {
        ...
        ActionDef(String l, String m, boolean p) {
            label = l;
            method = m;
            primary = p;
        }
    ...
    private static class CRUDView {
        VBox root;
        HBox actions;
        TableView<Object> table;
        Runnable reload;

        CRUDView(VBox r, HBox a, TableView<Object> t, Runnable x) {
            root = r;
            actions = a;
            table = t;
            reload = x;
    ...
    private interface RowActionFactory {
        Node create(TableView<Object> table, Object row);
    }
